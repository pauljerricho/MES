...
(function ($) {
  ...
  var barcode = {
    ...
    i25: { // std25 int25
      ...
      getDigit: function(code, crc, type){
        ...
        if (type == "int25") {
          ...
          // Standard 2 of 5 is a numeric-only barcode that has been in use a
			// long time.
          // Unlike Interleaved 2 of 5, all of the information is encoded in
			// the bars; the spaces are fixed width and are used only to
			// separate the bars.
    ...
    code11: {
      ...
      getDigit: function(code){
        ...
        var weightC    = 0,
        weightSumC = 0,
        weightK    = 1, // start at 1 because the right-most character is "C"
						// checksum
        weightSumK   = 0;
    ...
    code93:{
      ...
      getDigit: function(code, crc){
        var table = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-. $/+%____*", // _ =>
																		// ($),
																		// (%),
																		// (/)
																		// et
        ...
        if (crc){
          var weightC    = 0,
          weightSumC = 0,
          weightK    = 1, // start at 1 because the right-most character is
							// "C" checksum
          weightSumK   = 0;
          for(i=code.length-1; i>=0; i--){
    ...
    code128: {
      ...
      getDigit: function(code){
        ...
        while( i < code.length ){
          ...
          if ( tableCActivated ) {
            value = barcode.intval(code.charAt(i) + code.charAt(i+1)); // Add
																		// two
																		// characters
																		// (numeric)
            i += 2;
    ...
    datamatrix: {
      lengthRows:       [ 10, 12, 14, 16, 18, 20, 22, 24, 26,  // 24 squares et
																// 6 rectangular
                          32, 36, 40, 44, 48, 52, 64, 72, 80,  88, 96, 104, 120, 132, 144,
                          8, 8, 12, 12, 16, 16],
      lengthCols:       [ 10, 12, 14, 16, 18, 20, 22, 24, 26,  // Number of
																// columns for
																// the entire
																// datamatrix
                          32, 36, 40, 44, 48, 52, 64, 72, 80, 88, 96, 104, 120, 132, 144,
                          18, 32, 26, 36, 36, 48],
      dataCWCount:      [ 3, 5, 8, 12,  18,  22,  30,  36,  // Number of data
															// codewords for the
															// datamatrix
                          44, 62, 86, 114, 144, 174, 204, 280, 368, 456, 576, 696, 816, 1050, 
                          1304, 1558, 5, 10, 16, 22, 32, 49],
      solomonCWCount:   [ 5, 7, 10, 12, 14, 18, 20, 24, 28, // Number of
															// Reed-Solomon
															// codewords for the
															// datamatrix
                          36, 42, 48, 56, 68, 84, 112, 144, 192, 224, 272, 336, 408, 496, 620,
                          7, 11, 14, 18, 24, 28],
      dataRegionRows:   [ 8, 10, 12, 14, 16, 18, 20, 22, // Number of rows
															// per region
                          24, 14, 16, 18, 20, 22, 24, 14, 16, 18, 20, 22, 24, 18, 20, 22,
                          6,  6, 10, 10, 14, 14],
      dataRegionCols:   [ 8, 10, 12, 14, 16, 18, 20, 22, // Number of columns
															// per region
                          24, 14, 16, 18, 20, 22, 24, 14, 16, 18, 20, 22, 24, 18, 20, 22,
                          16, 14, 24, 16, 16, 22],
      regionRows:       [ 1, 1, 1, 1, 1, 1, 1, 1, // Number of regions per row
                          ...
                          1, 1, 1, 1, 1, 1],
      regionCols:       [ 1, 1, 1, 1, 1, 1, 1, 1, // Number of regions per
													// column
                          1, 2, 2, 2, 2, 2, 2, 4, 4, 4, 4, 4, 4, 6, 6, 6,
                          1, 2, 1, 2, 2, 2],
      interleavedBlocks:[ 1, 1, 1, 1, 1, 1, 1, 1, // Number of blocks
                          ...
                          1, 1, 1, 1, 1, 1],
      logTab:           [ -255, 255, 1, 240, 2, 225, 241, 53, 3,  // Table of
																	// log for
																	// the
																	// Galois
																	// field
      ...
      aLogTab:          [ 1, 2, 4, 8, 16, 32, 64, 128, 45, 90, // Table of aLog
																// for the
																// Galois field
                          ...
                          150, 1],
      champGaloisMult: function(a, b){  // MULTIPLICATION IN GALOIS FIELD
										// GF(2^8)
        if(!a || !b) return 0;
        return this.aLogTab[(this.logTab[a] + this.logTab[b]) % 255];
      },
      champGaloisDoub: function(a, b){  // THE OPERATION a * 2^b IN GALOIS FIELD
										// GF(2^8)
        if (!a) return 0;
        if (!b) return a;
      ...
      selectIndex: function(dataCodeWordsCount, rectangular){ // CHOOSE THE
																// GOOD INDEX
																// FOR TABLES
      ...
      calculSolFactorTable: function(solomonCWCount){ // CALCULATE THE REED
														// SOLOMON FACTORS
        var g = new Array();
        var i, j;
      ...
      addReedSolomonCW: function(nSolomonCW, coeffTab, nDataCW, dataTab, blocks){ // Add
																					// the
																					// Reed
																					// Solomon
																					// codewords
      ...
      next: function(etape, totalRows, totalCols, codeWordsBits, datamatrix, assigned){ // Place
																						// codewords
																						// into
																						// the
																						// matrix
        var chr = 0; // Place of the 8st bit from the first character to
						// [4][0]
        var row = 4;
        var col = 0;
      ...
      patternShapeStandard: function(datamatrix, assigned, bits, row, col, totalRows, totalCols){ // Place
																									// bits
																									// in
																									// the
																									// matrix
      ...
      placeBitInDatamatrix: function(datamatrix, assigned, bit, row, col, totalRows, totalCols){ // Put
																									// a
																									// bit
																									// into
																									// the
      ...
      addFinderPattern: function(datamatrix, rowsRegion, colsRegion, rowsRegionCW, colsRegionCW){ // Add
																									// the
																									// finder
																									// pattern
      ...
      getDigit: function(text, rectangular){
        var dataCodeWords = this.encodeDataCodeWordsASCII(text); // Code the
																	// text in
																	// the ASCII
																	// mode
        var dataCWCount = dataCodeWords.length;
        var index = this.selectIndex(dataCWCount, rectangular); // Select the
																// index for the
																// data tables
        var totalDataCWCount = this.dataCWCount[index]; // Number of data CW
        var solomonCWCount = this.solomonCWCount[index]; // Number of Reed
															// Solomon CW
        ...
        var blocks = this.interleavedBlocks[index];  // Number of Reed
														// Solomon blocks
        var errorBlocks = (solomonCWCount / blocks);
        
        this.addPadCW(dataCodeWords, dataCWCount, totalDataCWCount); // Add
																		// codewords
																		// pads
        
        var g = this.calculSolFactorTable(errorBlocks); // Calculate correction
														// coefficients
        
        this.addReedSolomonCW(solomonCWCount, g, totalDataCWCount, dataCodeWords, blocks); // Add
																							// Reed
																							// Solomon
																							// codewords
    ...
    digitToBmpRenderer: function($container, settings, digit, hri, mw, mh){
      ...
      var padding = (4 - ((mw * columns * 3) % 4)) % 4; // Padding for 4 byte
														// alignment ("* 3" come
														// from "3 byte to color
														// R, G and B")
      ...
      var bmp = 'BM' +                            // Magic Number
                this.lec.cInt(54 + dataLen, 4) +  // Size of Bitmap size
													// (header size + data len)
                '\0\0\0\0' +                      // Unused
                this.lec.cInt(54, 4) +            // The offset where the
													// bitmap data (pixels) can
													// be found
                this.lec.cInt(40, 4) +            // The number of bytes in
													// the header (from this
													// point).
                this.lec.cInt(mw * columns, 4) +  // width
                this.lec.cInt(mh * lines, 4) +    // height
                this.lec.cInt(1, 2) +             // Number of color planes
													// being used
                this.lec.cInt(24, 2) +            // The number of bits/pixel
                '\0\0\0\0' +                      // BI_RGB, No compression
													// used
                this.lec.cInt(dataLen, 4) +       // The size of the raw BMP
													// data (after this header)
                this.lec.cInt(2835, 4) +          // The horizontal resolution
													// of the image
													// (pixels/meter)
                this.lec.cInt(2835, 4) +          // The vertical resolution
													// of the image
													// (pixels/meter)
                this.lec.cInt(0, 4) +             // Number of colors in the
													// palette
                this.lec.cInt(0, 4);              // Means all colors are
													// important
      // Bitmap Data
    ...
    digitToBmp: function($container, settings, digit, hri,title){
      var w = barcode.intval(settings.barWidth);
      var h = barcode.intval(settings.barHeight);
      this.digitToBmpRenderer($container, settings, this.bitStringTo2DArray(digit), hri, w, h,title);
    },
    // bmp 2D barcode renderer
    digitToBmp2D: function($container, settings, digit, hri,title){
      var s = barcode.intval(settings.moduleSize);
      this.digitToBmpRenderer($container, settings, digit, hri, s, s,title);
    },
    // css barcode renderer
    digitToCssRenderer : function($container, settings, digit, hri, mw, mh,title){
      var lines = digit.length;
      var columns = digit[0].length;
      var content = "";
      ...
      if (settings.showHRI){
		content += "<div style=\"clear:both; width: 100%; background-color: " + settings.bgColor + "; color: " + settings.color + "; text-align: center; font-size: " + settings.fontSize + "px; margin-top: " + settings.marginHRI + "px;\">"+title+"</div>";
      }
	  var len, current;
    ...
    digitToCss: function($container, settings, digit, hri,title){
      var w = barcode.intval(settings.barWidth);
      var h = barcode.intval(settings.barHeight);
      this.digitToCssRenderer($container, settings, this.bitStringTo2DArray(digit), hri, w, h,title);
    },
    // css 2D barcode renderer
    digitToCss2D: function($container, settings, digit, hri,title){
      var s = barcode.intval(settings.moduleSize);
      this.digitToCssRenderer($container, settings, digit, hri, s, s,title);
    },
  ...
  $.fn.extend({
    barcode: function(datas, type, settings,title_data) {
      var digit = "",
          hri   = "",
          code  = "",
          crc   = true,
      ...
      if (typeof(barcode[fname]) == 'function') {
        barcode[fname]($this, settings, digit, hri,title);
      }
