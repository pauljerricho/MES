...
list($data,$total_record,$row_page) = $total_idata_row->total_idata_row_("*","members","","num desc","","10","10");

if($total_record){
    foreach ($data as $key=>$row){
?>
			<li>
				<form method="post" action="adm_mem_udt.php">
				<input type="hidden" name="num" value="<?=$row['num']?>">
					<span class="col1"><?=$row['article_num']?></span>
					<span class="col2"><?=$row['id']?></a></span>
					<span class="col3"><?=$row['name']?></span>
					<span class="col4"><input type="text" name="level" value="<?=$row['level']?>"></span>
					<span class="col5"><input type="text" name="point" value="<?=$row['point']?>"></span>
					<span class="col6"><?=$row['regist_day']?></span>
					<span class="col7"><button type="submit" class="fontstyle">MODIFY</button></span>&nbsp;&nbsp;&nbsp;
					<span class="col8"><button type="button" onclick="location.href='adm_mem_del.php?num=<?=$row['num']?>'" class="fontstyle">DELETE</button></span>
				</form>
			</li>
<?php 
    }
