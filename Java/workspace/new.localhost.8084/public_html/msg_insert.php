...
if(!$num_record){
    $message->message_("$num_record,수신 아이디가 잘못되었습니다.","history.back();","exit");
}else{
    $lib->lib_fun("dbi_insert");
    $sql = $dbi_insert->dbi_insert_("message","send_id,rv_id,subject,content,regist_day","'$userid','$rv_id','$subject','$content','$regist_day'");
}
